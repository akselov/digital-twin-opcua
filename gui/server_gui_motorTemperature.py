...
# Plot figure
def plot_fig(xmin, xmax, ymin, ymax):
    plt.axis([xmin, xmax, ymin, ymax])
    plt.title('Current motor temperature of KUKA KR 16-2, Axis 1-6')
    plt.grid(True)
    plt.xlabel('time [s*10]')
    plt.ylabel('Motor temperature [' + u'\u2103' + ']')
    plt.ion()

    # Construct a colormap
    cmap = clr.LinearSegmentedColormap.from_list('cmap for Dennis Hein', [(0, '#ff0000'), (70/100., '#ffff00'), (100/100., '#00ff00')], N=64)
    X = [[.0, .0], [1.0, 1.0]]
    plt.imshow(X, interpolation='bicubic', cmap = cmap, extent=(xmin, xmax, ymin, ymax), alpha = 0.3)
...
if __name__ == "__main__":
   # Initialize OPC-UA Client connection
    ...
    # Start plot
    xmin, xmax, ymin, ymax = 0, 200, 15, 100
    plot_fig(xmin, xmax, ymin, ymax)
    ...
    try:
        ...
        while True:
            ...
            plotted_values_A1.append(float(robot.read("$MOT_TEMP [1]")) - offset)
            plotted_values_A2.append(float(robot.read("$MOT_TEMP [2]")) - offset)
            plotted_values_A3.append(float(robot.read("$MOT_TEMP [3]")) - offset)
            plotted_values_A4.append(float(robot.read("$MOT_TEMP [4]")) - offset)
            plotted_values_A5.append(float(robot.read("$MOT_TEMP [5]")) - offset)
            plotted_values_A6.append(float(robot.read("$MOT_TEMP [6]")) - offset)
